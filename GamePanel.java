...
public class GamePanel extends JPanel implements Runnable, KeyListener {
    ...
    private void initGameObjects() {
        ...
        // <-- CORRECCIÓN: primero ponemos los bounds para que reset use el tamaño correcto
        ball.setBounds(width, height);
        ball.reset(rand.nextBoolean() ? 1 : -1);
    }
    ...
    private void updateGame() {
        // Siempre actualizar estrellas y paddles para que sigan moviéndose
        for (Star s : stars) {
            s.x += s.speed;
            if (s.x > width) {
                s.x = -5;
        ...
        if (!isPaused) {
            ...
            if (ball.intersects(leftPaddle)) ball.bounceOffPaddle(leftPaddle);
            else if (ball.intersects(rightPaddle)) ball.bounceOffPaddle(rightPaddle);
